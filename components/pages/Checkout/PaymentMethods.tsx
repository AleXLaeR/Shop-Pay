    ...
    <div className="payment">
      ...
      {paymentMethods.map(({ id, name, description, images }) => (
        <label
          ...
          htmlFor={id}
          role="presentation"
          className={`p-4 flex items-center gap-2.5 cursor-pointer rounded-md mt-4 hover:bg-white-dark ${
            paymentMethod === id ? 'bg-white-darker' : ''
          }`}
          onClick={() => setPaymentMethod(id)}
        >
          <input type="radio" name="payment" id={id} checked={paymentMethod === id} title={name} />
          <Image
